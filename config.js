 ...
 * DASHSCOPE_MODEL 决定（默认 qwen-turbo）；仅在不走代理、直连 Gemini/OpenAI 时使用下方 model。
 */
...
/**
 * 结局1（怒气辞职）第二屏：打字机多段正文。
 * - 按数组顺序逐段打出；每一段结束后都会播点阵「……」，再进入下一段。
 * - 可任意增删改字符串；空串会被忽略。若整表无效则回退为两句默认。
 */
window.GAME_ENDING_RAGE_TYPEWRITER_LINES = [
  "于是", 
  "你选择了诗和远方",
  "走向了大理和西藏",
  "洗了洗心肝脾肺肾",
